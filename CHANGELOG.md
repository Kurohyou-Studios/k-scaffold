...
## Changelog
### 2.8.1
- **Dynamic select options: `+dynamicOptions` mixin.** Usable inside a `+select` block alongside `+option`, it populates options at runtime (Roll20 only, via `populateListOptions`). Two forms: declarative `+dynamicOptions({section:'inventory',label:'name'})` creates one option per row of the named repeating section (value = row id, label = that row's attribute), and `+dynamicOptions({function:'myGenFn',trigger:{affects:[...]}, section?})` calls a registered option generator function returning `{value,label}[]`. Dynamic options are inserted in author order relative to the static options. Label changes, row removal, and row addition rebuild the list automatically for the declarative form (the fieldset's `addFuncs`/`triggeredFuncs` and the label attribute's `triggeredFuncs` are injected at build time); every dynamic select is also rebuilt on `sheet:opened` since Roll20 does not persist `populateListOptions` results. When the selected option disappears the select resets to its fallback static option (the `selected` one, else the first). Build-time errors: no static `+option` in the select, target section declared with plain `+fieldset` instead of `+customControlFieldset`/`+repeating_section`, unknown section or label attribute, unknown `trigger.affects` attribute, or use inside a repeating section. See [ADR 0020](docs/adr/0020-dynamic-select-options.md).
- **Sheetworkers: `k.updateDynamicOptions(selectName,{trigger,attributes,sections,casc})`** rebuilds one dynamic select on demand.
- **Tests: added `populateListOptions` mock to `mock20.js`.** Calls are recorded and the requested option lists are kept in `environment.listOptions` indexed by selector.
