...
const kAddNestedItem = function({trigger, attributes, sections}) {
    ...
    console.log({event: 'kAddNestedItem', section, selector, level, sections, rowid, newChildID});
    return newChildID;
};
...
const _getRowIDFromNestedTrigger = function(name) {
    ...
    if (name.startsWith('repeating')) {
        const [match] = (name.match(/^repeating_[^_]+_(.+)_add---.+$/) || []).slice(1);
        rowid = match;
    } else {
    ...
    console.log({event: '_getRowIDFromNestedTrigger', name, rowid});
    return rowid;
};
