...
const _kAddNestedItem = function({trigger, attributes, sections}) {
    k.log('hello !');
    const section = trigger.data.sectionName;
    const selector = trigger.data.selectorName;
    const level = trigger.data.levelQualname;
    const rowid = _getRowIDFromNestedTrigger(trigger.name);
    console.log({section, selector, rowid, level});
...
k.registerFuncs({_kAddNestedItem});


const _getRowIDFromNestedTrigger = function(name) {
    if (trigger.name.startsWith('repeating')) {
        const [rowid] = (trigger.name.match(/^repeating_[^_]+_(.+)_add---.+$/) || []).slice(1);
        return rowid;
    } else {
        // The sheetworker was called from a top-level action button, outside the repeating
        // section
        return '-'.concat(_nestedLevelPlaceholderChar.repeat(_Roll20RowIDLength));
    }
};
...
const _getNextAvailableNestedChildren = function(rowId, sectionRowIds) {
    console.log('Calling _getNextAvailableNestedChildren');
    console.log({rowId, sectionRowIds});
    ...
    console.log({placeholderIndex, prefix});
    ...
    console.log({childrenCharCodes, childrenIds});
