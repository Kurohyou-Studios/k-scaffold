...
/**
 * Derives the canonical cascade name for an element from its `name` property and the current repeating section prefix. The element's `title` is never consulted, so user-supplied titles (e.g. custom tooltips) do not affect the cascade.
 * @memberof Render
 * @param {object} element - Object describing the element
 * @returns {string}
 */
const cascadeName = (element) => {
  const baseName = `${element.name || ''}`.replace(/^(?:attr|act|roll)_/,'');
  const fullName = element.type === 'fieldset' ?
    baseName :
    `${k.repeatingPrefix}${baseName}`;
...
const storeTrigger = function(element){
  ...
  trigger.name = cascadeName(element);
...
module.exports = { varObjects, k, resetObjs, checkKUse, getSystemPrefix, setSystemPrefix, attrTitle, attrName, buttonTitle, replaceSpaces, replaceProblems, capitalize, actionButtonName, actionInputName, titleToName, addIfUnique, cascadeName, storeTrigger, getSectionDetails, createFieldsetObj, addFieldToFieldsetObj, makeElementObj };