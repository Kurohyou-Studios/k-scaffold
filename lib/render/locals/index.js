...
const defaultKObject = () => {
  return {
    ...
    styles:{},
    fonts:[],
    customControlSections:[],
    pendingCustomControl:false
  };
}
/**
...
/**
 * Records that a repeating section was declared with a custom add control (customControlFieldset), which is the only way row additions can be detected on Roll20.
 * @memberof Render
 * @param {string} section - The full name of the repeating section including the `repeating_` prefix.
 */
const markCustomControlSection = function(section){
  if(k.customControlSections.indexOf(section) < 0){
    k.customControlSections.push(section);
  }
};

/**
 * Converts a repeating section reference as an author would write it in a mixin argument (`inventory`, `repeating_inventory`, `my section`) into the candidate full names it may have been declared under, applying the current system prefix the same way {@link fieldset} does. The candidates are matched case-insensitively against the declared sections by {@link finalizeDynamicOptions}.
 * @memberof Render
 * @param {string} section
 * @returns {string[]}
 */
const sectionNameCandidates = (section) => {
  const bare = `${section}`.replace(/^repeating_/,'');
  const prefix = getSystemPrefix();
  const names = prefix && !k.repeatsIgnoreSystemPrefix ?
    [`${prefix} ${bare}`,bare] :
    [bare];
  return names.map(n => `repeating_${n.replace(/\s+/g,'-')}`);
};

const dynamicOptionsFunc = 'kDynamicOptionsUpdate';

/**
 * Converts the argument of a `+dynamicOptions` call into the normalized entry stored in the dynamic options registry. Throws when the argument is not one of the two supported forms.
 * @memberof Render
 * @param {object} dynObj - The object passed to the dynamicOptions mixin
 * @param {string} selectName - The attribute name of the select the mixin was used in
 * @returns {object}
 */
const dynamicOptionEntry = function(dynObj,selectName){
  const {section,label,trigger} = dynObj;
  const fn = dynObj.function;
  if(fn){
    if(typeof fn !== 'string'){
      throw new Error(`+dynamicOptions() in "${selectName}": "function" must be the name of an option generator function registered with k.registerFuncs.`);
    }
    const entry = {type:'function',function:fn};
    if(section){
      entry.section = sectionNameCandidates(section);
    }
    entry.affects = (trigger?.affects || []).map(a => replaceSpaces(`${a}`).toLowerCase());
    return entry;
  }
  if(section){
    if(!label){
      throw new Error(`+dynamicOptions() in "${selectName}": the declarative form requires both "section" and "label" (the attribute in that section to use as the option label).`);
    }
    return {type:'section',section:sectionNameCandidates(section),label:replaceSpaces(`${label}`).toLowerCase()};
  }
  throw new Error(`+dynamicOptions() in "${selectName}" requires either a "section" (declarative form) or a "function" (option generator form).`);
};

/**
 * Stores the description of a select whose options are populated at runtime. Validation that needs the whole sheet (does the section exist, is it a customControlFieldset) is deferred to {@link finalizeDynamicOptions}.
 * @memberof Render
 * @param {object} config - `{name, selector, fallback, entries}`
 */
const registerDynamicOptions = function(config){
  varObjects.dynamicOptions[config.name] = config;
};

/**
 * Validates every registered dynamic option source against the finished sheet and injects the wiring the runtime needs into the cascade: the rebuild function is added as a `triggeredFuncs` entry on each declarative label attribute and each option generator dependency, and as `addFuncs`/`triggeredFuncs` on each referenced fieldset. Section references are resolved to the declared section name here (case-insensitively) so the sheetworkers see the same name they get from `getSectionIDs`. Called by the kscript mixin, which is the last mixin in a sheet.
 * @memberof Render
 */
const finalizeDynamicOptions = function(){
  const resolveSection = (candidates,selectName) => {
    const declared = varObjects.repeatingSectionDetails.map(d => d.section);
    const match = candidates
      .map(c => declared.find(d => d.toLowerCase() === c.toLowerCase()))
      .find(d => d);
    if(!match){
      throw new Error(`+dynamicOptions() in "${selectName}" references the repeating section "${candidates[candidates.length - 1]}", but no fieldset with that name exists on the sheet.`);
    }
    if(k.customControlSections.indexOf(match) < 0){
      throw new Error(`+dynamicOptions() in "${selectName}" targets "${match}", which is declared with +fieldset. Roll20 only reports row additions for sections declared with +customControlFieldset (or +repeating_section); declare "${match}" with one of those instead.`);
    }
    return match;
  };
  Object.values(varObjects.dynamicOptions).forEach((config) => {
    config.entries.forEach((entry) => {
      if(entry.section){
        entry.section = resolveSection(entry.section,config.name);
        storeTrigger({name:entry.section,type:'fieldset',trigger:{addFuncs:[dynamicOptionsFunc],triggeredFuncs:[dynamicOptionsFunc]}});
      }
      if(entry.type === 'section'){
        const labelAttr = `${entry.section}_$X_${entry.label}`;
        if(!varObjects.cascades[`attr_${labelAttr.toLowerCase()}`]){
          throw new Error(`+dynamicOptions() in "${config.name}": no attribute named "${entry.label}" exists in "${entry.section}" to use as the option label.`);
        }
        storeTrigger({name:labelAttr,trigger:{triggeredFuncs:[dynamicOptionsFunc]}});
      }
      if(entry.type === 'function'){
        entry.affects.forEach((dep) => {
          if(!varObjects.cascades[`attr_${dep}`]){
            throw new Error(`+dynamicOptions() in "${config.name}": trigger.affects names "${dep}", but no attribute with that name exists on the sheet.`);
          }
          storeTrigger({name:dep,trigger:{triggeredFuncs:[dynamicOptionsFunc]}});
        });
      }
    });
  });
...
module.exports = { varObjects, k, resetObjs, checkKUse, getSystemPrefix, setSystemPrefix, attrTitle, attrName, buttonTitle, replaceSpaces, replaceProblems, capitalize, actionButtonName, actionInputName, titleToName, addIfUnique, storeTrigger, getSectionDetails, createFieldsetObj, addFieldToFieldsetObj, makeElementObj, markCustomControlSection, sectionNameCandidates, dynamicOptionEntry, registerDynamicOptions, finalizeDynamicOptions };