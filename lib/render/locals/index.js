...
/**
 * Normalizes a repeating section reference (`inventory`, `repeating_inventory`, `my section`) to the full `repeating_` name used in the cascade.
 * @memberof Render
 * @param {string} section
 * @returns {string}
 */
const normalizeSectionName = (section) => `repeating_${`${section}`.replace(/^repeating_/,'').replace(/\s+/g,'-')}`;
...
const dynamicOptionEntry = function(dynObj,selectName){
  ...
  if(fn){
    if(typeof fn !== 'string'){
      throw new Error(`+dynamicOptions() in "${selectName}": "function" must be the name of a function registered with k.registerFuncs.`);
    }
    const entry = {type:'function',function:fn};
    if(section){
      entry.section = normalizeSectionName(section);
    }
  ...
  if(section){
    ...
    return {type:'section',section:normalizeSectionName(section),label:replaceSpaces(`${label}`).toLowerCase()};
  }
  throw new Error(`+dynamicOptions() in "${selectName}" requires either a "section" (declarative form) or a "function" (custom generator form).`);
};
...
const registerDynamicOptions = function(config){
  varObjects.dynamicOptions = varObjects.dynamicOptions || {};
  varObjects.dynamicOptions[config.name] = config;
};

/**
 * Validates every registered dynamic option source against the finished sheet and injects the wiring the runtime needs into the cascade: the rebuild function is added as a `triggeredFuncs` entry on each declarative label attribute and each custom generator dependency, and as `addFuncs`/`triggeredFuncs` on each referenced fieldset. Called by the kscript mixin, which is the last mixin in a sheet.
 * @memberof Render
 */
const finalizeDynamicOptions = function(){
  const addFunc = (cascName,prop) => {
    const cascObj = varObjects.cascades[cascName];
    cascObj[prop] = [...new Set([...(cascObj[prop] || []),dynamicOptionsFunc])];
  };
  Object.values(varObjects.dynamicOptions || {}).forEach((config) => {
    config.entries.forEach((entry) => {
      if(entry.section){
        if(!getSectionDetails(entry.section)){
          throw new Error(`+dynamicOptions() in "${config.name}" references the repeating section "${entry.section}", but no fieldset with that name exists on the sheet.`);
        }
        if(k.customControlSections.indexOf(entry.section) < 0){
          throw new Error(`+dynamicOptions() in "${config.name}" targets "${entry.section}", which is declared with +fieldset. Roll20 only reports row additions for sections declared with +customControlFieldset (or +repeating_section); declare "${entry.section}" with one of those instead.`);
        }
        storeTrigger({name:entry.section,type:'fieldset',trigger:{addFuncs:[dynamicOptionsFunc],triggeredFuncs:[dynamicOptionsFunc]}});
      }
      if(entry.type === 'section'){
        const cascName = `attr_${entry.section}_$x_${entry.label}`;
        ...
        addFunc(cascName,'triggeredFuncs');
      }
      if(entry.type === 'function'){
        entry.affects.forEach((dep) => {
          const cascName = `attr_${dep}`;
          if(!varObjects.cascades[cascName]){
