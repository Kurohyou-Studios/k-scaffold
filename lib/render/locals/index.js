...
const resetObjs = () =>{
  ...
  Object.assign(varObjects,JSON.parse(JSON.stringify(varTemplate)));

  Object.keys(k).forEach((key)=>delete k[key]);
  Object.assign(k,JSON.parse(JSON.stringify(kTemplate)));
};
...
const storeTrigger = function(element){
  ...
  if(cascObj.formula){
    ...
    const formulaObj = {
      name:trigger.name,
      contents:attrCalls.map(call => call.replace(/@{|}/g,''))
    };
  ...
  if(Array.isArray(trigger.affects)){
    cascObj.affects = cascObj.affects.map((affect)=>replaceSpaces(affect));
  }
  varObjects.cascades[cascName] = cascObj;
