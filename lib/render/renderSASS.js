      ...
      importers: [
        {
          findFileUrl(url) {
            ...
            const fileURL = pathToFileURL(path.posix.resolve(__dirname, '../..'), url.substring(10));
        ...
        },
        {
          canonicalize(url) {
            ...
            const fileURL = pathToFileURL(path.posix.resolve(dirname, `/${url}.scss`), url.substring(10));
...
@mixin roll {${sfcStyles.roll || '/*No roll sfc used*/'}}`,
        ...
        },
        {
          ...
          load(canonicalUrl) {
            return {
              contents:
                `@import url("https://fonts.googleapis.com/css?family=Material+Icons|Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200${sfcFonts.length ?
                  `|${sfcFonts.join('|')}` :
                  ''
                }&display=swap");`,
              syntax: 'scss'
