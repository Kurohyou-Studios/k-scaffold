...
const kWatch = ({source ='./',destination,dynamicDestination,testDestination,pugOptions={suppressStack:true},scssOptions={}}) => {
  watch(source, 
    {
      recursive: true,
      filter(f, skip) {
    ...
    async (evt,name)=>{
      ...
      await processSheet({source,destination,dynamicDestination,testDestination,pugOptions,scssOptions,runSCSS,runPUG});
    });
};
