 ...
 */
const build = async ({source ='./',destination,dynamicDestination,testDestination,pugOptions={suppressStack:true},scssOptions={},watch=false}) => {
  const [html,css] = await processSheet({source,destination,dynamicDestination,testDestination,pugOptions,scssOptions});
  if(watch){
    return watchSheet({source,destination,dynamicDestination,testDestination,pugOptions,scssOptions});
  }else{
    return [html,css];
  }
};
