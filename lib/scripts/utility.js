...
/**
 * Parses a macro so that it is reduced to the final values of all attributes contained in the macro. Will drill down up to 99 levels deep. If the string was not parseable, the original string is returned.
...
const parseMacro = (str,attributes,sections) => {
  ...
  let parseable = true;
  let mutStr = str;
  while(mutStr.match(/@{.+?}/) && iter < 99 && parseable){
