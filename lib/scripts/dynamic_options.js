...
const dynamicOptionsFromEntry = function(entry,selectConfig,{trigger,attributes,sections,casc}){
  switch(entry.type){
    ...
    case 'section':
      // The label is the row's label attribute as-is; a row whose label is still blank shows a blank option until it is filled in.
...
const dynamicEntryDependsOn = function(entry,triggerName){
  ...
  if(section && triggerName === section){
    return true;
  }
  if(entry.type === 'section' && new RegExp(`^${sanitizeForRegex(section)}_[^_]+_${sanitizeForRegex(entry.label)}$`).test(triggerName)){
    return true;
  }
...
/**
 * Cascade function that the K-scaffold wires into label attributes, fieldsets, and option generator dependencies at compile time. Rebuilds every dynamic select whose option source depends on the trigger.
 * @memberof Dynamic Options
