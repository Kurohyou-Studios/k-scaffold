...
/**
 * Mock of Roll20's populateListOptions. Records the last option list requested for each selector in
 * `environment.listOptions` (indexed by elemSelector) and invokes the callback.
 */
const populateListOptions = vi.fn(({ elemSelector, optionsArray, callback, overwrite = true }) => {
  environment.listOptions = environment.listOptions || {};
  const existing = overwrite ? [] : (environment.listOptions[elemSelector] || []);
  environment.listOptions[elemSelector] = [...existing, ...optionsArray];
  if (typeof callback === 'function') callback();
});
global.populateListOptions = populateListOptions;
